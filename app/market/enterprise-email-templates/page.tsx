...
export default function EnterpriseEmailTemplatesPage() {
  const { user } = useUser()
  const [templates, setTemplates] = useState<EnterpriseEmailTemplate[]>([])
  ...
  const loadTemplates = async () => {
    if (!user) return
    setLoading(true)
    try { setTemplates(await loadEnterpriseEmailTemplates(user.userId)) }
  ...
  const handleSave = async () => {
    ...
    setSaving(true)
    try {
      if (editId) { await updateEnterpriseEmailTemplate(user.userId, editId, form); setEditId(null) }
      else await createEnterpriseEmailTemplate(user.userId, form)
  ...
  const handleDelete = async (id: string) => {
    ...
    try { await deleteEnterpriseEmailTemplate(user.userId, id); await loadTemplates() }
  ...
  return (
    <div className="container mx-auto px-4 py-8">
      <h1 className="text-3xl font-bold mb-8">Enterprise Email Templates</h1>
