...
export default function VCEmailTemplatesPage() {
  const { user } = useUser()
  const [templates, setTemplates] = useState<VCEmailTemplate[]>([])
  ...
  const loadTemplates = async () => {
    if (!user) return
    setLoading(true)
    try { setTemplates(await loadVCEmailTemplates(user.userId)) }
  ...
  const handleSave = async () => {
    ...
    setSaving(true)
    try {
      if (editId) { await updateVCEmailTemplate(user.userId, editId, form); setEditId(null) }
      else await createVCEmailTemplate(user.userId, form)
  ...
  const handleDelete = async (id: string) => {
    ...
    try { await deleteVCEmailTemplate(user.userId, id); await loadTemplates() }
  ...
  return (
    <div className="container mx-auto px-4 py-8">
      <h1 className="text-3xl font-bold mb-8">VC Email Templates</h1>
