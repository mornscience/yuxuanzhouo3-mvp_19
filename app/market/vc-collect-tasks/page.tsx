...
export default function VCCollectTasksPage() {
  const router = useRouter()
  const [tasks, setTasks] = useState<any[]>([])
  const [loading, setLoading] = useState(true)
  const [error, setError] = useState<string | null>(null)
  const [newTask, setNewTask] = useState({ taskName: '', platform: 'Crunchbase', keyword: '', maxLimit: 1000 })
  ...
  const loadTasks = async () => {
    ...
    try {
      ...
      if (json.success) setTasks(json.data.vcCollectTasks || [])
  ...
  const handleCreate = async () => {
    ...
    try {
      const res = await fetch('/api/market/admin/acquisition', {
        ...
        body: JSON.stringify({ action: 'create_vc_collect_task', ...newTask })
      })
      const json = await res.json()
      if (json.success) {
        setNewTask({ taskName: '', platform: 'Crunchbase', keyword: '', maxLimit: 1000 })
        await loadTasks()
  ...
  const handleUpdateStatus = async (taskId: string, status: string) => {
    try {
      await fetch('/api/market/admin/acquisition', {
        method: 'POST', credentials: 'include',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ action: 'update_vc_task_status', taskId, status })
      })
      await loadTasks()
            ...
            <div>
              ...
              <Select value={newTask.platform} onValueChange={v => setNewTask({...newTask, platform: v})}>
                ...
                <SelectContent>
                  ...
                  <SelectItem value="IT桔子">IT桔子</SelectItem>
                  <SelectItem value="投资界">投资界</SelectItem>
                  <SelectItem value="36氪创投">36氪创投</SelectItem>
                </SelectContent>
              </Select>
            </div>
            <div><Label>{t("invest_keyword")}</Label><Input value={newTask.keyword} onChange={e => setNewTask({...newTask, keyword: e.target.value})} placeholder="e.g. AI, Healthcare, Clean Energy" /></div>
              ...
              <TableRow key={task.id}>
                ...
                <TableCell>
                  <div className="flex gap-2">
                    <Button size="sm" variant="outline" onClick={() => router.push(`/market/vc-collect-temp?taskId=${task.id}`)}>{t("view_data")}</Button>
